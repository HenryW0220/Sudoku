...
export default function SudokuCell(props: any) {
    ...
    const handleCellClick = () => {
        sudokuCellSelected(row, col);
    };

    return (
        <div onClick={handleCellClick} className={`outline outline-1 p-1 size-16 
            ${shaded ? "bg-purple-100" : "bg-white"} flex items-center hover:bg-slate-200 transition-color duration-150
            ${row === 1 && col === 1 && "rounded-tl-3xl"}
            ${row === 1 && col === 9 && "rounded-tr-3xl"}
            ${row === 9 && col === 1 && "rounded-bl-3xl"}
            ${row === 9 && col === 9 && "rounded-br-3xl"}`}>
